import { playerlistEx } from './usernamePreGame.js'
...
const shuffleArray = listActivePlayer => {
    for (let i = listActivePlayer.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i * 1));
        const temp = listActivePlayer[i];
        listActivePlayer[i] = listActivePlayer[j]
        listActivePlayer[j] = temp;
